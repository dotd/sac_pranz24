...
def collect_train_trajectories(config: Config, env: Union[gym.Env, StationaryCheetahWindVelEnv], train_buffer: VAEBuffer):
    ...
    for trajectory_idx in range(config.train_buffer.max_episode_num):
        ...
        obs, actions, rewards, next_obs, dones = sample_trajectory(env=env, max_env_steps=config.train_buffer.max_episode_len)
...
def collect_test_trajectories(config: Config, env: Union[gym.Env, StationaryCheetahWindVelEnv], buffer: VAEBuffer):

    for trajectory_idx in range(config.test_buffer.max_episode_num):

        env.set_task(task=None)

        obs, actions, rewards, next_obs, dones = sample_trajectory(env=env, max_env_steps=config.test_buffer.max_episode_len)
...
def collect_stationary_trajectories(env: Union[gym.Env,
    ...
    for trajectory_idx in range(episode_num):
        ...
        obs, actions, rewards, next_obs, dones = sample_trajectory(env=env, max_env_steps=episode_len)
